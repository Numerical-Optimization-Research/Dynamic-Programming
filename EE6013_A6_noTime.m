...
%% Variables
global a  b c d e kT Tref delta_t kSoC socRef ...
    kt socMax socMin dodMin dodMax ...
    minVal
...
for i = 1:time_max
    ...
    fd_final = evalFd(xMin(:,i));
    deg_dot = -kt*exp(kSoC*(xMin(3,i) - socRef))*exp(kT*(xMin(2,i) - Tref)*(Tref/xMin(2,i)))*exp(-fd_final);
    cost_out(i) = -Ceol*deg_dot;
    
end

figure('name','Optimum Cost');
subplot(2,1,1)
plot(1:time_max,cost_out,'LineWidth',1.5);
xlim([0 50]);
grid on;
xlabel('Time index');
...
subplot(2,1,2)
plot(1:time_max,xMin(1,:),1:time_max,xMin(2,:),1:time_max,xMin(3,:),'LineWidth',1.5);
grid on;
ylim([0 1]);
xlim([0 50]);
xlabel('Time index');
